...
import { StackEditor } from '@/blockkit/StackEditor';
import { StackRegistry, createDefaultRegistry } from '@/blockkit/StackRegistry';
import { StackProgram, StackBlock } from '@/blockkit/types';
...
function getSequenceElement(id: string): HTMLElement {
  const el = document.querySelector(`[data-sequence-id="${id}"]`) as HTMLElement | null;
  if (!el) {
    throw new Error(`Sequence element ${id} not found`);
  }
  return el;
}
...
describe('StackEditor Drag and Drop', () => {
  ...
  describe('Block creation from palette drag operations', () => {
    it('should create new block when dragging from palette to main sequence', () => {
      ...
      const dropZone = getSequenceElement('main');
    ...
    it('should create new block when dragging from palette to C-block slot', () => {
      ...
      const slotDropZone = getSequenceElement('slot:repeat1:DO');
  ...
  describe('Block movement within and between sequences', () => {
    it('should move block within the same main sequence', () => {
      ...
      const mainSequence = getSequenceElement('main');
      ...
      fireEvent.dragOver(mainSequence, { dataTransfer: dragStartEvent.dataTransfer });
    ...
    it('should move block from main sequence to C-block slot', () => {
      ...
      const slotArea = getSequenceElement('slot:existing-repeat1:DO');
      ...
      fireEvent.dragOver(slotArea, { dataTransfer: dragStartEvent.dataTransfer });
    ...
    it('should move block between different C-block slots', () => {
      ...
      const thenSlot = getSequenceElement('slot:if1:THEN');
      ...
      fireEvent.dragOver(thenSlot, { dataTransfer: dragStartEvent.dataTransfer });
  ...
  describe('Verify no artificial movement restrictions exist', () => {
    ...
    it('should allow C-blocks to be nested inside other C-blocks', () => {
      ...
      const outerSlot = getSequenceElement('slot:outer-repeat:DO');
      ...
      fireEvent.dragOver(outerSlot, { dataTransfer: dragStartEvent.dataTransfer });
